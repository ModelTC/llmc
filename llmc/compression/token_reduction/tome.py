        ...
        for idx, block in enumerate(self.blocks):
            if self.r[idx] > 0:
                ...
                if isinstance(block, CLIPEncoderLayer):  # llava
                    ...
                    block.self_attn.forward = types.MethodType(tome_CLIPSdpaAttention_forward, block.self_attn)  
                    block.original_forward = block.forward  
                    block.forward = types.MethodType(tome_CLIPEncoderLayer_forward, block)  
                # elif isinstance(block, Qwen2VLVisionBlock): # qwenvl
                #     block.self_attn.original_forward = block.self_attn.forward  
                #     block.self_attn.forward = types.MethodType(tome_VisionSdpaAttention_forward, block.self_attn)  
                #     block.original_forward = block.forward  
                #     block.forward = types.MethodType(tome_Qwen2VLVisionBlock_forward, block)  
                ...
                #     block.attn.forward = types.MethodType(tome_InternAttention_forward, block.attn)  
                #     block.original_forward = block.forward  
                #     block.forward = types.MethodType(tome_InternVisionEncoderLayer_forward, block)  
...
def tome_Qwen2VLVisionBlock_forward(self, hidden_states, cu_seqlens, rotary_pos_emb) -> torch.Tensor:
    residual = hidden_states
...
def tome_InternAttention_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
    x, key_mean = self._naive_attn(hidden_states) if not self.use_flash_attn else self._flash_attn(hidden_states)
    return x, key_mean
