...
class BaseBlockwiseQuantization(BlockwiseOpt):
    ...
    def set_quant_config(self):
            ...
            for i in range(len(mix_bits_settings)):
                ...
                if mix_bits_setting["do_quant"]:
                    ...
                    if "act" in mix_bits_setting:
                        ...
                        aquantizer_mix_bits = Quantizer(**self.quant_config["act"])
                    else:
                    ...
                    self.quantizer_mix_bits.append(
                        {
                            ...
                            "aquantizer": aquantizer_mix_bits if not w_only_mix_bits else None
                        }
                    )
                else:
                    self.quantizer_mix_bits.append(
        ...
        for i in range(len(self.quantizer_mix_bits)):
            ...
            for name in layer_name:
                ...
                assert (len(n_layeridx) == 1 or len(n_layeridx) == 2), "layer_name in mix_bits must be name#1-3-4 or name."
                if len(n_layeridx) == 2:
                    n = n_layeridx[0]
                ...
                if layeridx == "all":
                    for k in range(self.num_blocks):
                        ...
                        # self.mix_bits_map[k]['layer_idx'] = k
                else:
                    for k in layeridx:
                        self.mix_bits_map[k][n] = i
                        # self.mix_bits_map[k]['layer_idx'] = k

        logger.info(f"self.mix_bits_map:\n{json.dumps(self.mix_bits_map, ensure_ascii=False, indent=4)}")
    ...
    def block_opt(self, block):
        ...
        if self.quant_out:
            ...
            self.model.replace_module_block(FakeQuantLinear, block, self.block_idx, params_dict)
            self.input["data"] = self.block_forward(block)
    ...
    def apply_clip(self, layer, min_val, max_val, layer_name):
        if self.clip_version == "v1":
            ...
            up_factor, low_factor = self.get_clip_factor(layer, min_val, max_val, layer_name)
            layer.register_buffer("buf_upbound_factor", up_factor)
            layer.register_buffer("buf_lowbound_factor", low_factor)
            if self.save_clip:
                layer_name = f"{self.model.block_name_prefix}.{self.block_idx}.{layer_name}"
    ...
    def get_clip_factor(self, layer, min_val, max_val, layer_name):
        wquantizer = get_wquantizer(self.block_idx, layer_name, self.mix_bits_map, self.quantizer_mix_bits, self.wquantizer)
    ...
    @torch.no_grad()
    def auto_clip_layer(
        self, layer_name, w, input, n_grid=20, max_shrink=0.5, n_sample_token=512, eps=0.0
    ):
        assert w.dim() == 2
    ...
    def deploy(self, quant_format):
        ...
        if quant_format == "fake_quant":
            ...
            if "mix_bits" not in self.quant_config:
                params_dict["a_qdq"] = self.a_qdq if not self.w_only else None
