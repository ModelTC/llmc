...
device_zbl = "cpu"  # cuda & cpu
use_cuda = (device_zbl!="cpu")

def main(config):
    # tokenizer,model 加载在cpu上
    ...
    # 一个或者多个数据集上进行eval，对每个数据集进行 初始化 ppl_eval
    ...
    sparsification = None
    if not config.get("calib", False):  # 如果不进行校准，则直接使用指定的量化方法进行量化。
        ...
        calib_data = dataset.get_calib_dataset()   # 1*60*512
        model.collect_first_block_input(calib_data)  # 1*60*512
        del calib_data
        gc.collect()
        if use_cuda:
